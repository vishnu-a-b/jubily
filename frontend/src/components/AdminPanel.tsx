...
export default function AdminPanel({ username }: AdminPanelProps) {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (!editForm.name.trim() || !editForm.mobileNo.trim()) {
      setToast({ message: 'Name and mobile number are required', type: 'error' });
      return;
    }

    if (!/^\d{10}$/.test(editForm.mobileNo)) {
      setToast({ message: 'Invalid mobile number format (10 digits required)', type: 'error' });
      return;
    }

    try {
      await registrationAPI.updateRegistration(editingRegistration.couponNo, {
        name: editForm.name.trim(),
        mobileNo: editForm.mobileNo.trim()
      });
      setToast({ message: 'Registration updated successfully', type: 'success' });
      setEditingRegistration(null);
      setEditForm({ name: '', mobileNo: '' });
      loadRegistrations();
    } catch (error: any) {
            ...
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
              ...
              <form onSubmit={handleEditSubmit}>
                <div className="mb-4">
                  ...
                  <input
                    type="text"
                    value={editingRegistration.couponNo}
                    disabled
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600 cursor-not-allowed"
                  />
                  <p className="text-xs text-gray-500 mt-1">Coupon number cannot be changed</p>
                </div>
